...
using static Aitive.Framework.SourceGenerators.Framework.Output.WellKnownChars;
...
public sealed class SourceWriter
{
    ...
    internal SourceWriter(char indentationChar, int charsPerIndentation, bool cacheToString = true)
    {
    ...
    /// </summary>
    /// <param name="cacheToString">Whether the <see cref="ToString"/> result must be cached or not.</param>
    internal SourceWriter(bool cacheToString = true)
        : this(' ', 4, cacheToString) { }
    ...
    internal SourceWriter OpenBlock()
    {
        WriteLine(OpenBrace);
        Indentation++;
        return this;
    ...
    /// </summary>
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal SourceWriter CloseBlock()
    {
        Indentation--;
    ...
    /// </summary>
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal SourceWriter CloseAllBlocks()
    {
        while (Indentation > 0)
        {
            CloseBlock();
    ...
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal SourceWriter WriteEmptyLines(int linesCount = 1)
    {
    ...
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal SourceWriter WriteLine(char value)
    {
        AddIndentation();
    ...
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal unsafe SourceWriter WriteLine(string text)
    {
        if (_indentation == 0)
        {
            _sb.AppendLine(text);
        ...
        do
        {
            ...
            AddIndentation();
            fixed (char* ptr = nextLine)
            {
                _sb.Append(ptr, nextLine.Length);
            }

            StringBuilder temp = _sb;
            _sb.AppendLine();
        } while (!isFinalLine);
    ...
    /// </summary>
    /// <returns>A self <see cref="SourceWriter"/> instance to chain calls.</returns>
    internal SourceWriter WriteLine()
    {
        _sb.AppendLine();
        return this;
    }
    ...
    internal SourceText ToSourceText()
    {
        Debug.Assert(_indentation == 0);
        return SourceText.From(ToString(), Encoding.UTF8);
    }

    internal SourceFile ToSourceFile(string filename)
    {
        Debug.Assert(_indentation == 0);
        return new SourceFile(filename, ToString());
    }

    /// <inheritdoc />
    public override string ToString() => _useCachedToString ? GetCachedToString() : _sb.ToString();
