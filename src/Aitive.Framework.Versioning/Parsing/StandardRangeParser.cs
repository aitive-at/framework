...
internal static class StandardRangeParser
{
    ...
    )
    {
        ...
        if (segment.IsEmpty())
            return ex ?? RangeError.MissingComparison(segment.Offset, segment.Buffer!);

        var start = LeftBoundedRange.Unbounded;
        var end = RightBoundedRange.Unbounded;
        var includeAllPrerelease = rangeOptions.HasOption(
            SemVersionRangeOptions.IncludeAllPrerelease
        );
        while (!segment.IsEmpty())
        {
            exception = ParseComparison(
                ref segment,
                rangeOptions,
                ref includeAllPrerelease,
                ex,
                maxLength,
                ref start,
                ref end
    ...
    )
    {
        ...
        exception = GeneralRangeParser.ParseVersion(
            ref segment,
            rangeOptions,
            ParsingOptions,
            ex,
        ...
        switch (@operator)
        {
            ...
            case StandardOperator.Caret:
                ...
                rightBound = rightBound.Min(
                    new RightBoundedRange(
                        new SemVersion(
                            major,
                            minor,
                            patch,
                ...
                rightBound = rightBound.Min(
                    new RightBoundedRange(
                        semver.With(
                            minor: semver.Minor + BigInteger.One,
                            patch: BigInteger.Zero,
                            prerelease: PrereleaseIdentifiers.Zero
                        ),
                        false
                    )
                );
            ...
            case StandardOperator.None: // implied = (supports wildcard *)
                ...
                if (wildcardVersion != WildcardVersion.None && semver.IsPrerelease)
                    return ex
                        ?? RangeError.PrereleaseNotSupportedWithWildcardVersion(segment.Buffer!);
    ...
    private static LeftBoundedRange WildcardLowerBound(SemVersion semver, bool prereleaseWildcard)
    {
        if (prereleaseWildcard)
            semver = semver.IsPrerelease
                ? semver.WithPrerelease(
                    semver.PrereleaseIdentifiers.Concat(PrereleaseIdentifiers.Zero)
                )
    ...
    private static Exception? ParseOperator(
        ref StringSegment segment,
        Exception? ex,
        out StandardOperator @operator
    )
    {
        var end = 0;
        while (
            end < segment.Length
            && GeneralRangeParser.IsPossibleOperatorChar(
                segment[end],
                SemVersionRangeOptions.Strict
            )
        )
            end++;
        ...
        @operator = 0;
        if (opSegment.Length > 2 || (opSegment.Length == 2 && opSegment[1] != '='))
    ...
    private static readonly SemVersionParsingOptions ParsingOptions = new(
        true,
        true,
        false,
        c => c == '*'
