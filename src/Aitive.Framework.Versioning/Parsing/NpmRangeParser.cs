...
internal static class NpmRangeParser
{
    ...
    )
    {
        ...
        {
            var exception = ParseHyphenRange(
                segment1,
                segment2,
                rangeOptions,
    ...
    )
    {
        ...
        int i;
        while (
            start < segment.Length && (i = segment.IndexOf('-', start, searchLength - start)) >= 0
        )
        {
            var indexBefore = i - 1;
            var indexAfter = i + 1;
    ...
    {
        var exception = ParseHyphenSegment(
            beforeHyphenSegment,
            rangeOptions,
            ex,
            maxLength,
            out var semver1,
            out var wildcardVersion1
        );
        if (exception != null)
            return exception;
        DebugChecks.IsNotNull(semver1, nameof(semver1));

        exception = ParseHyphenSegment(
            afterHyphenSegment,
            rangeOptions,
            ex,
            maxLength,
            out var semver2,
            out var wildcardVersion2
        );
        if (exception != null)
            return exception;
    ...
    {
        var exception = GeneralRangeParser.ParseVersion(
            ref segment,
            rangeOptions,
            ParsingOptions,
            ex,
    ...
    )
    {
        ...
        switch (wildcardVersion)
        {
            ...
            case WildcardVersion.MinorPatchWildcard:
            {
                ...
                var prerelease = includeAllPrerelease
                    ? PrereleaseIdentifiers.Zero
