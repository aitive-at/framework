...
using Aitive.Framework.Versioning.Utilities;
...
internal static class SemVersionParser
{
    ...
    private const string LeadingZeroInMajorMinorOrPatchMessage =
        "{1} version has leading zero in '{0}'.";
    ...
    private const string PrereleasePrefixedByDotMessage =
        "The prerelease identfiers should be prefixed by '-' instead of '.' in '{0}'.";
    private const string MissingPrereleaseIdentifierMessage =
        "Missing prerelease identifier in '{0}'.";
    private const string LeadingZeroInPrereleaseMessage =
        "Leading zero in prerelease identifier in version '{0}'.";
    private const string InvalidCharacterInPrereleaseMessage =
        "Invalid character '{1}' in prerelease identifier in '{0}'.";
    private const string MissingMetadataIdentifierMessage = "Missing metadata identifier in '{0}'.";
    private const string InvalidCharacterInMajorMinorOrPatchMessage =
        "{1} version contains invalid character '{2}' in '{0}'.";
    private const string InvalidCharacterInMetadataMessage =
        "Invalid character '{1}' in metadata identifier in '{0}'.";
    private const string InvalidWildcardInMajorMinorOrPatchMessage =
        "{1} version is a wildcard and should contain only 1 character in '{0}'.";
    private const string MinorOrPatchMustBeWildcardVersionMessage =
        "{1} version should be a wildcard because the preceding version is a wildcard in '{0}'.";
    ...
    private const string PrereleaseWildcardMustBeLastMessage =
        "Prerelease identifier follows wildcard prerelease identifier in '{0}'.";
    private const string MajorMinorOrPatchParsingFailedMessage =
        "{1} version '{2}' failed to parse in '{0}'.";
    private const string PrereleaseParsingFailedMessage =
        "Prerelease identifier '{1}' failed to parse in '{0}'.";
    ...
    )
    {
        ...
        if (version.Length == 0)
            return ex ?? new FormatException(EmptyVersionMessage);
        ...
        mainSegment.SplitBeforeFirst(
            '-',
            out var majorMinorPatchSegment,
            out var prereleaseSegment
        );
        ...
        using (var versionNumbers = majorMinorPatchSegment.Split('.').GetEnumerator())
        {
            ...
            parseEx = ParseVersionNumber(
                "Major",
                version,
                versionNumbers,
                allowLeadingZeros,
                majorIsOptional,
                majorIsWildcardRequired,
                options,
                ex,
                out major,
                out var majorIsWildcard
            );
            if (parseEx != null)
                return parseEx;
            if (majorIsWildcard)
                wildcardVersion |= WildcardVersion.MajorMinorPatchWildcard;

            var minorIsOptional =
                style.HasStyle(SemVersionStyles.OptionalMinorPatch) || majorIsWildcard;
            parseEx = ParseVersionNumber(
                "Minor",
                version,
                versionNumbers,
                ...
                ex,
                out minor,
                out var minorIsWildcard
            );
            if (parseEx != null)
                return parseEx;
            if (minorIsWildcard)
                wildcardVersion |= WildcardVersion.MinorPatchWildcard;

            var patchIsOptional =
                style.HasStyle(SemVersionStyles.OptionalPatch)
                || majorIsWildcard
                || minorIsWildcard;
            parseEx = ParseVersionNumber(
                "Patch",
                ...
                ex,
                out patch,
                out var patchIsWildcard
            );
            if (parseEx != null)
                return parseEx;
            if (patchIsWildcard)
                wildcardVersion |= WildcardVersion.PatchWildcard;
            ...
            if (versionNumbers.MoveNext())
            {
                ...
                return ex
                    ?? NewFormatException(
                        PrereleasePrefixedByDotMessage,
                        version.ToStringLimitLength()
                    );
            }
        }
        ...
        if (prereleaseSegment.Length > 0)
        {
            ...
            parseEx = ParsePrerelease(
                ...
                ex,
                out prerelease,
                out prereleaseIdentifiers,
                out var prereleaseIsWildcard
            );
            if (parseEx != null)
                return parseEx;
            DebugChecks.IsNotNull(prerelease, nameof(prerelease));
            if (prereleaseIsWildcard)
                wildcardVersion |= WildcardVersion.PrereleaseWildcard;
        }
        else
        {
            prerelease = "";
        ...
        if (metadataSegment.Length > 0)
        {
            ...
            parseEx = ParseMetadata(
                version,
                metadataSegment,
                ex,
                out metadata,
                out metadataIdentifiers
            );
            if (parseEx != null)
                return parseEx;
        }
        ...
        if (hasTrailingWhitespace && !style.HasStyle(SemVersionStyles.AllowTrailingWhitespace))
            return ex
                ?? NewFormatException(TrailingWhitespaceMessage, version.ToStringLimitLength());

        semver = new SemVersion(
            major,
            minor,
            patch,
            prerelease,
    ...
    )
    {
        ...
        if (oldLength > segment.Length && !style.HasStyle(SemVersionStyles.AllowLeadingWhitespace))
            return ex
                ?? NewFormatException(LeadingWhitespaceMessage, version.ToStringLimitLength());
    ...
    )
    {
        ...
        switch (leadChar)
        {
            ...
            case 'v':
                return ex
                    ?? NewFormatException(LeadingLowerVMessage, version.ToStringLimitLength());
            case 'V' when style.HasStyle(SemVersionStyles.AllowUpperV):
                segment = segment.Subsegment(1);
                break;
            case 'V':
                return ex
                    ?? NewFormatException(LeadingUpperVMessage, version.ToStringLimitLength());
        }
    ...
    )
    {
        ...
        if (
            options.AllowWildcardMajorMinorPatch
            && segment.Length > 0
            && options.IsWildcard(segment[0])
        )
        {
            isWildcard = true;
            if (segment.Length > 1)
                return ex
                    ?? NewFormatException(
                        InvalidWildcardInMajorMinorOrPatchMessage,
                        version.ToStringLimitLength(),
                        kind
                    );
        ...
        if (wildcardRequired)
            return ex
                ?? NewFormatException(
                    MinorOrPatchMustBeWildcardVersionMessage,
                    version.ToStringLimitLength(),
                    kind
                );
        ...
        // Scan for digits
        var i = 0;
        while (i < segment.Length && segment[i].IsDigit())
            i += 1;
        ...
        if (i < segment.Length)
            return ex
                ?? NewFormatException(
                    InvalidCharacterInMajorMinorOrPatchMessage,
                    version.ToStringLimitLength(),
                    kind,
                    segment[i]
                );
        ...
        if (
            ...
            return ex
                ?? new InvalidOperationException(
                    string.Format(
                        CultureInfo.InvariantCulture,
                        MajorMinorOrPatchParsingFailedMessage,
                        version.ToStringLimitLength(),
                        kind,
                        numberString
                    )
    ...
    )
    {
        ...
        foreach (var identifier in segment.Split('.'))
        {
            ...
            if (isWildcard)
                return ex
                    ?? NewFormatException(
                        PrereleaseWildcardMustBeLastMessage,
                        version.ToStringLimitLength()
                    );
            ...
            if (identifier.Length == 0)
                return ex
                    ?? NewFormatException(
                        MissingPrereleaseIdentifierMessage,
                        version.ToStringLimitLength()
                    );
            ...
            for (int i = 0; i < identifier.Length; i++)
            {
                ...
                if (c.IsAlphaOrHyphen())
                    ...
                    return ex
                        ?? NewFormatException(
                            InvalidCharacterInPrereleaseMessage,
                            version.ToStringLimitLength(),
                            c
                        );
            }
            ...
            if (isWildcard)
            {
                if (identifier.Length > 1)
                    return ex
                        ?? NewFormatException(
                            InvalidWildcardInPrereleaseMessage,
                            version.ToStringLimitLength()
                        );
                isWildcard = true;
                ...
                if (
                    ...
                    return ex
                        ?? new FormatException(
                            string.Format(
                                CultureInfo.InvariantCulture,
                                PrereleaseParsingFailedMessage,
                                version.ToStringLimitLength(),
                                identifier
                            )
                        );
        ...
        prerelease =
            hasLeadingZeros || isWildcard ? string.Join(".", identifiers) : segment.ToString();
    ...
    )
    {
        ...
        foreach (var identifier in segment.Split('.'))
        {
            ...
            if (identifier.Length == 0)
                return ex
                    ?? NewFormatException(
                        MissingMetadataIdentifierMessage,
                        version.ToStringLimitLength()
                    );
            ...
            for (int i = 0; i < identifier.Length; i++)
            {
                ...
                if (!c.IsAlphaOrHyphen() && !c.IsDigit())
                    return ex
                        ?? NewFormatException(
                            InvalidCharacterInMetadataMessage,
                            version.ToStringLimitLength(),
                            c
                        );
            }
    ...
    private static FormatException NewFormatException(
