...
using Aitive.Framework.Versioning.Ranges;
...
public sealed class UnbrokenSemVersionRange : IEquatable<UnbrokenSemVersionRange>
{
    ...
    public static UnbrokenSemVersionRange Empty { get; } =
        new UnbrokenSemVersionRange(
            new LeftBoundedRange(null, false),
    ...
    public static UnbrokenSemVersionRange AllRelease { get; } =
        Create(null, false, null, false, false);
    ...
    public static UnbrokenSemVersionRange GreaterThan(
        SemVersion version,
        bool includeAllPrerelease = false
    ...
    public static UnbrokenSemVersionRange AtLeast(
        SemVersion version,
        bool includeAllPrerelease = false
    ...
    public static UnbrokenSemVersionRange LessThan(
        SemVersion version,
        bool includeAllPrerelease = false
    ...
    public static UnbrokenSemVersionRange AtMost(
        SemVersion version,
        bool includeAllPrerelease = false
    ...
    public static UnbrokenSemVersionRange Inclusive(
        SemVersion start,
        SemVersion end,
        bool includeAllPrerelease = false
    ) =>
    ...
    /// not the end.</returns>
    public static UnbrokenSemVersionRange InclusiveOfStart(
        SemVersion start,
        SemVersion end,
        bool includeAllPrerelease = false
    ) =>
    ...
    public static UnbrokenSemVersionRange InclusiveOfEnd(
        SemVersion start,
        SemVersion end,
        bool includeAllPrerelease = false
    ) =>
    ...
    /// <returns>A range containing versions between the given versions including the end but
    /// not the start.</returns>
    public static UnbrokenSemVersionRange Exclusive(
        SemVersion start,
        SemVersion end,
        bool includeAllPrerelease = false
    ) =>
    ...
    )
    {
        ...
        if (start.Version is not null && end.Version is not null)
        {
            ...
            else if (start.IncludesPrerelease || end.IncludesPrerelease)
            {
                if (start.Version.MajorMinorPatchEquals(end.Version))
                    ...
                    (end.IncludesPrerelease || end.Version.PrereleaseIsZero)
                    && start.Version.Major == end.Version.Major
        ...
        return new UnbrokenSemVersionRange(
            start,
            end,
            includeAllPrerelease,
            allPrereleaseCoveredByEnds
        );
    }
    ...
    private string ToStringInternal()
    {
        ...
        var includesPrereleaseNotCoveredByEnds =
            IncludeAllPrerelease && !allPrereleaseCoveredByEnds;
    ...
    private bool TryToSpecialString(
        bool includesPrereleaseNotCoveredByEnds,
    ...
    )
    {
        ...
        if (StartInclusive && !RightBound.Inclusive && End?.PrereleaseIsZero == true)
        {
            ...
            )
            {
                if (Start.Major == End.Major && Start.Minor + BigInteger.One == End.Minor)
                    ...
                    Start.Major + BigInteger.One == End.Major
                    && Start.Minor == 0
                    && End.Minor == 0
                )
                    // Wildcard minor
            ...
            if (
                Start.PrereleaseIsZero
                && Start.Major == End.Major
            ...
            if (
                Start.Major == End.Major
                && Start.Minor + BigInteger.One == End.Minor
                && End.Patch == 0
            )
            {
        ...
        )
        {
            ...
            if (leftPrerelease[^2].NextIdentifier() != rightPrerelease[^1])
                return false;

            var originalPrerelease = string.Join(
                ".",
                leftPrerelease.Take(leftPrerelease.Count - 1)
            );
    ...
    internal bool OverlapsOrAbuts(UnbrokenSemVersionRange other)
    {
        ...
        if (
            (leftRangeEnd.Inclusive || rightRangeStart.Inclusive)
        ...
        if (
            IncludeAllPrerelease
            || other.IncludeAllPrerelease
            || leftRangeEnd.IncludesPrerelease
            || rightRangeStart.IncludesPrerelease
        )
            return false;
    ...
    internal bool Contains(UnbrokenSemVersionRange other)
    {
        ...
        {
            if (!(Start?.IsPrerelease ?? false) || !Start.MajorMinorPatchEquals(other.Start))
                return false;
        }
    ...
    /// </summary>
    internal bool TryUnion(
        UnbrokenSemVersionRange other,
        [NotNullWhen(true)] out UnbrokenSemVersionRange? union
    )
    {
        ...
        if (!includeAllPrerelease)
        {
            ...
            if (
                otherLeftBound.IncludesPrerelease && !possibleUnion.Contains(otherLeftBound.Version)
            )
                return false;

            var otherRightBound = RightBound.Min(other.RightBound);
            if (
                otherRightBound.IncludesPrerelease
    ...
    )
    {
        ...
        if (
            ...
            || end.Inclusive
            || includeAllPrerelease
            || start.Version.IsPrerelease
            || end.Version.IsPrerelease
        )
            return false;
    ...
    private static SemVersion Validate(SemVersion version, string paramName)
    {
        ...
        if (version.MetadataIdentifiers.Count > 0)
            throw new ArgumentException(InvalidMetadataMessage, paramName);
        return version;
    }
