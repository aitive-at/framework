...
public readonly struct PrereleaseIdentifier
    : IEquatable<PrereleaseIdentifier>,
        IComparable<PrereleaseIdentifier>,
        IComparable
{
    ...
    internal static PrereleaseIdentifier CreateUnsafe(string value, BigInteger? numericValue)
    {
        ...
        if (expected.NumericValue != numericValue)
            throw new ArgumentException(
                $"DEBUG: Numeric value {numericValue} doesn't match string value.",
                nameof(numericValue)
            );
#endif
    ...
    internal PrereleaseIdentifier(string value, bool allowLeadingZeros, string paramName)
    {
        ...
        if (value.IsDigits())
        {
            if (value.Length > 1 && value[0] == '0')
            {
                if (allowLeadingZeros)
                    ...
                    throw new ArgumentException(
                        $"Leading zeros are not allowed on numeric prerelease identifiers '{value}'.",
                        paramName
                    );
            }
        ...
        else
        {
            if (!value.IsAlphanumericOrHyphens())
                throw new ArgumentException(
                    $"A prerelease identifier can contain only ASCII alphanumeric characters and hyphens '{value}'.",
                    paramName
                );
            NumericValue = null;
        }
    ...
    public PrereleaseIdentifier(BigInteger value)
    {
        if (value < 0)
            throw new ArgumentOutOfRangeException(
                nameof(value),
                $"Numeric prerelease identifiers can't be negative: {value}."
            );
    ...
    {
        if (NumericValue is BigInteger numericValue)
            return HashCode.Combine(numericValue);
    ...
    public int CompareTo(object? value)
    {
        ...
        return value is PrereleaseIdentifier other
            ? CompareTo(other)
            : throw new ArgumentException(
                $"Object must be of type {nameof(PrereleaseIdentifier)}.",
                nameof(value)
            );
    }
    #endregion
