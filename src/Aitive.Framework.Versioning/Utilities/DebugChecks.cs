...
internal static class DebugChecks
{
    ...
    public static void IsValid(SemVersionStyles style, string paramName)
    {
        if (!style.IsValid())
            throw new ArgumentException(
                "DEBUG: " + SemVersion.InvalidSemVersionStylesMessage,
                paramName
            );
    }
    ...
    public static void IsValid(SemVersionRangeOptions rangeOptions, string paramName)
    {
        if (!rangeOptions.IsValid())
            throw new ArgumentException(
                "DEBUG: " + SemVersionRange.InvalidOptionsMessage,
                paramName
            );
    }
    ...
    [Conditional("DEBUG")]
    public static void IsNotWildcardVersionWithPrerelease(
    ...
    /// </summary>
    [Conditional("DEBUG")]
    public static void IsNotFailedException(
        Exception? exception,
        string className,
        string methodName
    )
    {
        if (exception != null && exception != VersionParsing.FailedException)
            throw new InvalidOperationException(
                $"DEBUG: {className}.{methodName} returned exception other than {nameof(VersionParsing.FailedException)}",
                exception
            );
    }
    ...
    public static void IsValidVersionNumber(BigInteger versionNumber, string kind, string paramName)
    {
        if (versionNumber < 0)
            throw new ArgumentException(
                $"DEBUG: {kind} version must be greater than or equal to zero.",
                paramName
            );
    }

    [Conditional("DEBUG")]
    public static void ContainsNoDefaultValues<T>(
        ...
        where T : struct
    {
        if (values.Any(i => EqualityComparer<T>.Default.Equals(i, default)))
            throw new ArgumentException(
                $"DEBUG: {kind} identifier cannot be default/null.",
                paramName
            );
    }

    [Conditional("DEBUG")]
    public static void AreEqualWhenJoinedWithDots<T>(
        string value,
        string param1Name,
    ...
    )
    {
        if (value != string.Join(".", values))
            throw new ArgumentException(
                $"DEBUG: must be equal to {param2Name} when joined with dots.",
                param1Name
            );
    }
}
