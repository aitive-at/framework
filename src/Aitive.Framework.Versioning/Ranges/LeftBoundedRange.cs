...
internal readonly struct LeftBoundedRange : IEquatable<LeftBoundedRange>
{
    ...
    public LeftBoundedRange(SemVersion? version, bool inclusive)
    {
        ...
        if (version is null && inclusive)
            throw new ArgumentException(
                "DEBUG: Cannot be inclusive of start without start value.",
                nameof(inclusive)
            );
        // dotcover enable
#endif
    ...
    public LeftBoundedRange Min(LeftBoundedRange other) => CompareTo(other) <= 0 ? this : other;

    public LeftBoundedRange Max(LeftBoundedRange other) => CompareTo(other) >= 0 ? this : other;

    #region Equality
    public bool Equals(LeftBoundedRange other) =>
        Equals(Version, other.Version) && Inclusive == other.Inclusive;

    public override bool Equals(object? obj) => obj is LeftBoundedRange other && Equals(other);
