...
namespace Aitive.Framework.Versioning.Ranges;
...
{
    public static readonly RightBoundedRange Unbounded = new RightBoundedRange(null, false);
    ...
    public RightBoundedRange(SemVersion? version, bool inclusive)
    {
        ...
        if (version is null && inclusive)
            throw new ArgumentException(
                "DEBUG: Cannot be inclusive of end without end value.",
                nameof(inclusive)
            );
        // dotcover enable
#endif
    ...
    public RightBoundedRange Min(RightBoundedRange other) => CompareTo(other) <= 0 ? this : other;

    public RightBoundedRange Max(RightBoundedRange other) => CompareTo(other) >= 0 ? this : other;

    #region Equality
    public bool Equals(RightBoundedRange other) =>
        Equals(Version, other.Version) && Inclusive == other.Inclusive;

    public override bool Equals(object? obj) => obj is RightBoundedRange other && Equals(other);

    public override int GetHashCode() => HashCode.Combine(Version, Inclusive);
