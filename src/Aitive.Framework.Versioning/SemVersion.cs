...
public sealed class SemVersion : IEquatable<SemVersion>, ISerializable, IParsable<SemVersion>
{
    ...
    internal const string InvalidSemVersionStylesMessage =
        "An invalid SemVersionStyles value was used.";
    private const string InvalidMajorVersionMessage =
        "Major version must be greater than or equal to zero.";
    private const string InvalidMinorVersionMessage =
        "Minor version must be greater than or equal to zero.";
    private const string InvalidPatchVersionMessage =
        "Patch version must be greater than or equal to zero.";
    private const string PrereleaseIdentifierIsDefaultMessage =
        "Prerelease identifier cannot be default/null.";
    private const string MetadataIdentifierIsDefaultMessage =
        "Metadata identifier cannot be default/null.";
        ...
        else
        {
            ...
            if (prereleaseIdentifiers.Any(i => i == default))
                throw new ArgumentException(
                    PrereleaseIdentifierIsDefaultMessage,
                    nameof(prerelease)
                );
        }
...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
    ...
    /// characters (i.e. characters that are not ASCII alphanumerics or hyphens).</exception>
    public static SemVersion ParsedFrom(
        BigInteger major,
    ...
    )
    {
        ...
        var metadataIdentifiers =
            metadata.Length == 0
                ? ReadOnlyList<MetadataIdentifier>.Empty
                : metadata.SplitAndMapToReadOnlyList(
                    '.',
    ...
    )
    {
        ...
        DebugChecks.ContainsNoDefaultValues(
            prereleaseIdentifiers,
            "Prerelease",
            nameof(prereleaseIdentifiers)
        );
        DebugChecks.AreEqualWhenJoinedWithDots(
            prerelease,
            nameof(prerelease),
            prereleaseIdentifiers,
            nameof(prereleaseIdentifiers)
    ...
    public static SemVersion FromVersion(Version version)
    {
        ...
        if (version.Revision > 0)
            throw new ArgumentException(
                "Version with Revision number can't be converted to SemVer.",
                nameof(version)
            );
    ...
    public Version ToVersion()
    {
        if (IsPrerelease)
            throw new InvalidOperationException(
                "Prerelease version can't be converted to System.Version."
            );
        if (Metadata.Length != 0)
            throw new InvalidOperationException(
                "Version with build metadata can't be converted to System.Version."
            );
        if (Major > int.MaxValue)
            throw new InvalidOperationException(
                string.Format(MajorMinorOrPatchVersionToLargeToConvertMessage, "major", Major)
            );
        if (Minor > int.MaxValue)
            throw new InvalidOperationException(
                string.Format(MajorMinorOrPatchVersionToLargeToConvertMessage, "minor", Minor)
            );
        if (Patch > int.MaxValue)
            throw new InvalidOperationException(
                string.Format(MajorMinorOrPatchVersionToLargeToConvertMessage, "patch", Patch)
            );
    ...
    public static SemVersion Parse(
        string version,
    ...
    {
        if (!style.IsValid())
            throw new ArgumentException(InvalidSemVersionStylesMessage, nameof(style));
        if (maxLength < 0)
            throw new ArgumentOutOfRangeException(nameof(maxLength), InvalidMaxLengthMessage);
    ...
    )
    {
        ...
        if (maxLength < 0)
            throw new ArgumentOutOfRangeException(nameof(maxLength), InvalidMaxLengthMessage);
        var exception = SemVersionParser.Parse(
            version,
            style,
            VersionParsing.FailedException,
            maxLength,
            out semver
        );

        DebugChecks.IsNotFailedException(
            exception,
            nameof(SemVersionParser),
            nameof(SemVersionParser.Parse)
        );
    ...
    public SemVersion WithPrerelease(
        ...
        params string[] prereleaseIdentifiers
    )
    {
        if (prereleaseIdentifier is null)
            throw new ArgumentNullException(nameof(prereleaseIdentifier));
        if (prereleaseIdentifiers is null)
        ...
        var identifiers = prereleaseIdentifiers
            ...
            .Select(i => new PrereleaseIdentifier(
                i,
    ...
    {
        if (metadataIdentifier is null)
            throw new ArgumentNullException(nameof(metadataIdentifier));
        if (metadataIdentifiers is null)
            throw new ArgumentNullException(nameof(metadataIdentifiers));
    ...
    )
    {
        ...
        if (identifiers.Any(i => i == default))
            throw new ArgumentException(
                MetadataIdentifierIsDefaultMessage,
                nameof(metadataIdentifiers)
            );
        return new SemVersion(
            Major,
    ...
    internal bool PrereleaseIsZero =>
        PrereleaseIdentifiers.Count == 1 && PrereleaseIdentifiers[0] == PrereleaseIdentifier.Zero;
    ...
    public override string ToString()
    {
        ...
        var estimatedLength =
            4 + major.Length + minor.Length + patch.Length + Prerelease.Length + Metadata.Length;
    ...
    /// <remarks>Two versions are equal if every part of the version numbers are equal. Thus, two
    /// versions with the same precedence may not be equal.</remarks>
    public override bool Equals(object? obj) => obj is SemVersion version && Equals(version);
    ...
    public static ISemVersionComparer PrecedenceComparer { get; } =
        Comparers.PrecedenceComparer.Instance;
    ...
    public static ISemVersionComparer SortOrderComparer { get; } =
        Comparers.SortOrderComparer.Instance;
